...
public class First_Queries
{
   // private static final File databaseDirectory = new File( "target/neo4j-query-1-6" );
    ...
    public static void main( final String[] args ) throws IOException
    {
    	...
    	First_Queries hello = new First_Queries();
    ...
    void Query1() { 
    	...
    	for (int i=0;i<CONSTANTS.iterations;i++) {
    		...
    		start = System.currentTimeMillis();
    	 	try ( 	Transaction ignored = graphDb.beginTx();
    	 			Result result = graphDb.execute("MATCH (X:Person{name:'Person1'})-[:RELATIONSHIP3]->(n1) With COLLECT(n1) as n MATCH (Y:Person{name:'Person2'})-[:RELATIONSHIP3]->(n1) WHERE n1 in n RETURN COUNT(DISTINCT(n1))"))
    	 	{
    	     	while ( result.hasNext() )
    	     	{
    	         	Map<String, Object> row = result.next();
     		...
     		timeIndex++;
	            ...
	            try ( Transaction tx = graphDb.beginTx() )
	            {
	            	...
	            	//System.out.println("Query 17:" + "Found new user " + friends1.size() );	
	            }
	     		end = System.currentTimeMillis();
	     		CONSTANTS.queries[timeIndex][i] = end - start ; 
	     		timeIndex++;
