...
Im Folgenden werden alle Einstellungen und Kategorisierungen, die für die Evaluation getroffen wurden, beschrieben. In den Testläufen werden Grundanfragen aufgestellt und umformuliert. So entstehen mehrere Anfragen, die betrachtet werden.  
\subsection{Der verwendete Datensatz}
Für das Experiment wurde ein selbsterstellter Datensatz gewählt, dieser aus einem selbst geschriebenen Programm erstellt \footnote{https://github.com/McHektor123/BA/tree/master/src}. Die Datenbank läuft im eingebettten Modus von Neo4j und besteht aus insgesamt 50.004 Entitäten, welche die Objekte der Datenbank bilden. 4 der Entitäten sind  vom Typ Aktion und 50000 vom Typ Person. Der Typ Aktion besitzt 3 mit einem Index versehende Attribute, mit den Namen $name$, welches aus einem String besteht, $attribute$, welches aus einem Int besteht und $time$, welches aus dem Typen Date besteht. Der Typ Person besitzt ebenfalls 3 mit einem Index versehende Attribute namens $name$, $attribute$ und $time$. Jeder Wert der einzelnen Attribute tritt genau einmal auf, sodass alle Werte disjunkt sind. \newline
...
Der erste Testlauf besteht primär aus Nachbarschafts-Anfragen und Zusammenfassungen, diese Anfragen sind einfache Beispiele, um einen ersten Überblick von Neo4j zu erhalten. Im folgenden Abschnitt werden die Grundanfragen des ersten Testlaufes und Umformulierungen der Grundanfragen vorgestellt. 
\subsection{Grundanfragen zu ersten Testlauf}
Das ist die \textbf{Grundanfrage 1.1}: 
\begin{Verbatim}[frame=single]
 MATCH (X:Person)-[:RELATIONSHIP3]->(Y:Person) 
 WHERE X.attribute>=250 AND Y.attribute>=15  
 RETURN COUNT(DISTINCT(Y))
\end{Verbatim} 
Grundanfrage 1.1 findet alle Personen, die über RELATIONSHIP3 erreichbar sind, unter der Voraussetzung dass bestimmte Bedingungen für die Eigenschaften 'attribute' erfüllt sind. Es handelt sich um eine Erreichbarkeits-Anfrage.
Als Vergleichsanfrage wird die Grundanfrage dahingehend umformuliert, dass die Richtung der betrachteten Relation von ausgehend zu eingehend geändert wird und eine weitere Vergleichsanfrage überprüft beide Richtungen. Zusätzlichen werden diese Anfragen  semantisch äquivalent in der JAVA Core API unter Verwendung der Traversal API formuliert. Bei den 3 in Cypher formulierten Anfragen wird für weitere Vergleichsanfragen die Relation RELATIONSHIP3 durch die Relation RELATIONSHIP2 ersetzt. Insgesamt entstehen 9 Anfragen aus Grundanfrage 1.1, dargestellt in Tabelle \ref{tab:Intro_Query2_1}.
\FloatBarrier
\begin{table}[h]
	\centering
	\begin{tabular}{ |p{3cm}||p{7cm}|p{3cm}  }
		...
		Anfrage& Name\\
		\hline
		1 &  Cypher - RELATIONSHIP3 ausgehend\\
		2 &  Cypher - RELATIONSHIP3 eingehend\\
		3 &  Cypher - RELATIONSHIP3 beides\\
		4 &  Cypher - RELATIONSHIP2 ausgehend\\
		5 &  Cypher - RELATIONSHIP2 eingehend\\
		6 &  Cypher - RELATIONSHIP2 beides\\
		7 &  Core API - RELATIONSHIP3 ausgehend\\
		8 &  Core API - RELATIONSHIP3 ausgehend\\
		9 &  Core API - RELATIONSHIP3 ausgehend\\
		\hline
	\end{tabular}
	\caption{Grundanfrage 2.1 und Vergleichsanfragen}
	\label{tab:Intro_Query2_1}
\end{table}
\FloatBarrier

Das ist die \textbf{Grundanfrage 1.2}: 
\begin{Verbatim}[frame=single]
...
\end{enumerate}
Zum 1. Aspekt: Die Ausführungszeit ist am höchsten, wenn die Relation in beide Richtung gelten kann, da dort am meisten Pfade betrachtet werden müssen. Die Zeit ist minimal wenn die Relation als ausgehende Kante beschrieben wird, da dort die Anzahl der zu betrachtenden Pfade minimal ist. \newline
...
Zum 3. Aspekt: Da jede Person zwei Relationen vom Typ RELATIONSHIP2 besitzt aber maximal 2.500 vom Typ RELATIONSHIP3 werden die Anfragen mit der Relation RELATIONSHIP2 mindestens um den Faktor 1.000 schneller ausgeführt werden. Dies entspricht einer Skalierung des Systems. \newline \newline
...
\begin{enumerate}
	\item Ein Performanz bei den Laufzeiten der semantisch äquivalenten Anfragen in Cypher
\end{enumerate}
...
\begin{table}[h]
	...
	\begin{tabular}{ |p{3cm}||p{5cm}|p{3cm}  }
		...
		1 &  Cypher - Tiefe 2\\
		2 &  Cypher - Tiefe 3\\
		3 &  Tiefensuche - Tiefe 2\\
		4 &  Tiefensuche - Tiefe 3\\
		5 &  Breitensuche - Tiefe 2\\
		6 &  Breitensuche - Tiefe 3\\
		\hline
	\end{tabular}
	\caption{Grundanfrage 2.1 und Vergleichsanfragen}
...
\end{table}
\FloatBarrier
Das ist die \textbf{Grundanfrage 2.2}: 
\begin{Verbatim}[frame=single]
...
\begin{enumerate}
	\item Ein Unterschied zwischen der Traversierung zu den Tiefen 2 und 3 
...
\end{enumerate}
Zum 1. Aspekt: Durch die Annahme, dass alle Personen über RELATIONSHIP3/ RELATIONSHIP4 mit der Tiefe 2 erreicht werden können, wird davon ausgegangen dass der Pfad die Länge 1 oder 2 besitzen  wird. Die Ergebnisse werden mit den Ausführungszeiten von Grundanfrage 2.1 zusammenhängen und es wird keine höhere Ausführungszeit als in Grundanfrage 2.1 erwartet. \newline
