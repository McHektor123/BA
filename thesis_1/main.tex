...
\begin{abstract}
Durch das steigende Interesse an der Analyse von Netzwerken  aus Daten werden Datenbanken des NoSQL-Ansatzes weiterentwickelt und für diese Analyse genutzt \parencite{angles2018property}. Eine Untergruppe dieses Ansatzes stellen die Graphdatenbanken dar, einer der populärsten Vertreter dieser Gruppe ist Neo4j. In dieser Arbeit wird beschrieben, wie sich Anfragen auf dem System Neo4j verhalten. Zu diesem Zweck wurden mehrere Anfragen mit semantisch gleichen Vergleichsanfragen an das System gestellt und die Differenz in den Bearbeitungszeiten wurde analysiert. Jede Anfrage wurde einer oder mehreren Kategorien zugeordnet, um so eine Korrelation zwischen Kategorie und Bearbeitungszeit einer Anfrage zu erkennen. \newline
Für eine Einordnung der Performanz von Neo4j wurden alle aufgestellten Anfragen ebenfalls auf dem Datenbankmanagementsystem OrientDB ausgeführt\footnote{http://orientdb.com/docs/3.0.x/misc/Overview.html (5.09.19)}. Die benötigten Zeiten für das Bearbeiten der Anfragen auf dem System OrientDB wurden mit den Bearbeitungszeiten der Anfragen auf Neo4j verglichen. Die Analyse zeigt, dass Neo4j die vorgestellten Anfragen mit der eigenen Anfragesprache Cypher durchschnittlich ca.1,9 mal schneller bearbeitet als OrientDB mit SQL. Neo4j kann alternativ in Java bedient werden und besitzt ein bessere Performanz als Cypher, sodass die Anfragen ca. 207 mal schneller als in OrientDB bearbeitet wurden. Es konnte keine allgemeine Korrelation zwischen Kategorie einer Anfrage und ihrer Bearbeitungszeit in Neo4j festgestellt werden. Für das Speichern des Datensatzes benötigt Neo4j ca. 2,4 mal mehr Speicherplatz als OrientDB. Mit der Verwendung dieser Referenzdatenbank werden die Nutzungsvielfalt und gute Performanz als Stärke und schlechte Speicherverwaltung als Schwäche von Neo4j hervorgehoben. \newline
\begin{center}
\textbf{Evaluation of temporal graph databases using the example of Neo4j}
\end{center}
Due to the increasing interest in analyzing networks of data, databases of the NoSQL approach are further developed and used for this analysis \parencite {angles2018property}. A subset of this approach is represented by the graph databases, one of the most popular members of this group is Neo4j. This work describes how queries behave on the system Neo4j. For this purpose, several queries with semantically equal comparison queries were processed on the system and the difference in the processing times was analyzed. Each query has been assigned to one or more categories to detect a correlation between the category and the processing time of a query. \newline
To rank the performance of Neo4j, all queries  were also processed on the database management system OrientDB \footnotemark[1]. The times needed to process queries on the system OrientDB were compared with the processing times of the queries on Neo4j. The analysis showed that Neo4j processes the queries presented with its own query language Cypher on average about 1.9 times faster than OrientDB with SQL. Neo4j can alternatively be operated in Java and has better performance than Cypher, so that the queries were processed about 207 times faster than in OrientDB. There was no general correlation between the category of a query and its processing time in Neo4j. Neo4j needs about 2.4 times more memory than the OrientDB to save the dataset. Using this reference database highlights the versatility and good performance as strength and poor memory management as a weakness of Neo4j.
\end{abstract}
